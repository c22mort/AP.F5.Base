#==================================================================================
# Script: 	Get-DeviceServerSideOutboundKb.ps1
# Date:		11/10/18
# Author: 	Andi Patrick
# Purpose:	Gets Device ServerSide Outbound Kb via SNMP returns as Property Bag
#==================================================================================
...
$SCRIPT_NAME			= 'Get-DeviceServerSideOutboundKb.ps1'
$EVENT_LEVEL_ERROR 		= 1
...
[string]$sysStatServerBytesOut5m = ".1.3.6.1.4.1.3375.2.1.1.2.1.97.0"
...
Try 
{
	...
	$oidClient = New-Object Lextm.SharpSnmpLib.ObjectIdentifier ($sysStatServerBytesOut5m)
	$vList.Add($oidClient)
	...
	Try {
		...
		if ($Results.Count -gt 0) {
			...
			[string] $message = "Created Property bag for $DeviceAddress`r`n" 	+ "Server-Side Outbound (Kb): "  + $NetworkKb + "`r`n"
